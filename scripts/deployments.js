...
module.exports = async function(robot) {
  ...
  await exec(
    ...
    } | base64 --decode > /gcloud-service-key.json`
  )
  await exec(
  ...
  await exec(
    `gcloud --quiet config set container/cluster ${
      process.env.GCLOUD_CLUSTER_NAME
    }`
  )
  await exec(
    `gcloud config set compute/zone ${process.env.GCLOUD_CLOUDSDK_COMPUTE_ZONE}`
  )
  ...
  let devopsChannel = channels.channels.filter(
    channel => channel.name === 'devops'
  )[0]
  ...
  async function transformUserToID(user) {
    ...
    users = users.members
    users = users.filter(
      u => u.profile.display_name === user || u.name === user || u.id === user
    )
    if (users.length === 0) {
  ...
  robot.hear(/!deployment admin add (.+)/, async res => {
    ...
    if (userIDToAdd && addUserWithRole(userIDToAdd, 'admin', brain)) {
      return res.send(`I added <@${userIDToAdd}> as a deployment admin.`)
    }
    return res.send(
  ...
  robot.hear(/!deployment add (.+) (.+)/, async res => {
    ...
    if (
      userIDToAdd &&
      addUserWithRole(userIDToAdd, `${where}Deployer`, brain)
    ) {
      return res.send(`I added <@${userIDToAdd}> as ${where} deployer.`)
    }
    return res.send(
  ...
  robot.hear(/!deployment remove (.+) (.+)/, async res => {
    ...
    const { brain } = robot
    const where = res.match[1].toLowerCase()
    if (where !== 'qa' && where !== 'staging' && where !== 'production') {
      return
    }
    const who = res.match[2]
    ...
    if (!isPrivateSlackMessage(res)) return
    if (!noAdmins(brain) && !isAdmin(user, brain)) return
    const [userLookupSuccess, userToRemove] = await transformUserToID(
      res.match[2]
    )
    if (!userLookupSuccess) {
    ...
    if (
      userToRemove &&
      removeUserWithRole(userToRemove, `${where}Deployer`, brain)
    ) {
      return res.send(`I removed <@${userToRemove}> as ${where} deployer.`)
    }
    return res.send(
  ...
  robot.hear(deployRegex, async msg => {
    ...
    if (matches[1] === 'qa') {
      ...
      if (!canDeploy(msg.message.user.id, 'qa', brain)) {
        return msg.send(
          'You do not have that permission, as far as I can see? Take it up with the admins...'
        )
      }
      await exec(
        'kubectl patch deployment dapp-red -p \'{"spec":{"template":{"spec":{"containers":[{"name":"dapp","image":"eu.gcr.io/fluent-aileron-128715/dapp-goerli:' +
          matches[2] +
          '"}]}}}}\''
      )
      ...
      await exec(
        'kubectl patch deployment dapp-' +
          colour +
          ' -p \'{"spec":{"template":{"spec":{"containers":[{"name":"dapp","image":"eu.gcr.io/fluent-aileron-128715/dapp-mainnet:' +
          matches[2] +
          '"}]}}}}\''
      )
    }
    msg.send(
      'Container patched. Could take up to two minutes to start responding.'
    )
  })
}
